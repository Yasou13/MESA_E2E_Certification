...
class SelfTestFailure(RuntimeError):
    """Raised when a self-test invariant fails, including under ``python -O``."""
...
def run_harness_self_tests() -> dict:
    ...
    _require(s1.status == "HIT" and s1.rank == 1 and s1.recall_at_1 == 1.0 and s1.recall_at_5 == 1.0 and s1.mrr == 1.0, "case 1 failed")
    ...
    _require(s2.status == "HIT" and s2.rank == 5 and s2.recall_at_1 == 0.0 and s2.recall_at_5 == 1.0 and abs(s2.mrr - 0.2) < 1e-4, "case 2 failed")
    ...
    _require(s3.status == "MISS" and s3.rank is None and s3.recall_at_1 == 0.0 and s3.recall_at_5 == 0.0 and s3.mrr == 0.0, "case 3 failed")
    ...
    _require(s4.status == "HIT" and s4.rank == 1 and len(s4.matching_chunk_ids) == 1, "case 4 failed")
    ...
    _require(s5.status == "HIT" and s5.rank == 1, "case 5 failed")
    ...
    _require(s6.status == "HIT" and s6.matching_chunk_ids == ["SRC-CANONICAL-001"], "case 6 failed")
    ...
    _require(s7.status == "HIT" and s7.complete_evidence_at_5 == 1.0 and s7.group_coverage_at_5 == 1.0, "case 7 failed")
    ...
    _require(s10.status == "INFRASTRUCTURE_ERROR" and s10.rank is None, "case 10 failed")
    ...
    _require("İSTANBUL Hukuk Fakültesi\nMadde 14" == t_norm, "case 15 text normalization failed")
    t_case = turkish_casefold("İSTANBUL IŞIK")
    _require(t_case == "istanbul ışık", "case 15 Turkish casefold failed")
    _require(normalize_law_id("4857 sayılı Kanun") == "4857", "case 15 law ID form 1 failed")
    _require(normalize_law_id("Kanun No: 4857") == "4857", "case 15 law ID form 2 failed")
    _require(normalize_article_id("Madde 14") == "madde-14", "case 15 article ID form 1 failed")
    _require(normalize_article_id("14. madde") == "madde-14", "case 15 article ID form 2 failed")
    ...
    _require(s16.status == "MISS" and s16.recall_at_5 == 0.0, "case 16 failed")
